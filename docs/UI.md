...
| A choice from a closed list | a native `<select>`; `.sheetsmith-card-select`, `.sheetsmith-table-select` | Card's options, Table's `level` column set to a select, Table's `target` column |
...
**Where a contributor line names the component as well as the row** is a rule
about the whole breakdown rather than about a line: a token that is the same on
every line carries no information and is dropped, so the component's label
appears only where a breakdown draws on more than one component, and then on
every line of it. The common sheet has one modifier table and reads
`Bag of Holding — item +2`; two of them, each with a row called "Ring", read
`Worn items · Ring — item +1`. Decided once per breakdown rather than per line, so
the shape changes only when the fact does.
...
| A modifier column's two-line detail row overruns the columns list's scroller, and the clip lands mid-column | `styles/editor.css` `.sheetsmith-list-scroll`, `editor/list-fields.ts` | `.sheetsmith-list-scroll` caps at a fixed `max-height: 20em` with no reason recorded. A column's detail line was one line until a modifier column put seven controls on it; the line now wraps — correctly, per §9's field row — so a modifier column costs two lines instead of one and a four-column table exceeds the cap. **Measured in `editor-modifiers` and `editor-modifiers-narrow`, and it is not only the narrow one:** at 1500 the cut falls *through* the last column's detail row rather than between rows — the **Minimum** and **Maximum** boxes and the **Bonus type** box holding `status` are all sliced horizontally and lose their bottom borders, three controls at once, while that column's **Hide heading** is gone entirely; its three checkboxes are the only part of the line that survives whole. At 1210 it loses everything below the Minimum/Maximum labels, so the column reads as one with no options rather than one with more below. So the wide case is the *worse* of the two to look at, not the safe one: a box with three sides reads as a rendering fault where a missing row reads as a shorter form. That is the failure `list-field-height.ts`'s header already describes for the textarea fields — "a line sliced through the middle of its glyphs does not read as *there is more below*, it reads as a rendering fault" — and on macOS, where overlay scrollbars appear only while scrolling, the clip is the only signal there is. What differs from the textarea case is where the cut falls: between a column's *row* and its *options*, so it reads as an absence rather than as an overflow. **Not fixed in the pass that found it.** The cap is shared by every list field in the panel — rows, columns, entries, track rows — so raising it changes the pane's whole rhythm, and it carries no comment saying what 20em was chosen against; the narrower fix, sizing this scroller to its content the way the textareas already do, is `list-field-height.ts`'s argument applied to a second kind of field and is a change of its own. **Waiting on:** a decision on what that cap is for, taken with the pane in front of whoever takes it — the two shots named above are what it should be judged against. |
