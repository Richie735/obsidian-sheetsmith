...
focused control, forced colors, and the empty and error states of both the sheet
and the editor — a vault with no layouts, and a layout file that will not parse
— so a review can look at PNGs rather than clicking through.
...
| At the reflow threshold the panel is wider than the grid beside it | `styles/editor.css` | `grid-template-columns: minmax(0, 1fr) 620px` pins the panel and gives the left column the remainder, so at the 1176px threshold the schematic sits at its 540px floor — about 45px a track for twelve columns — beside a four-field form holding 620px. Measured at a 1210px window, which is 1186px of pane. The left column only overtakes the panel above roughly 1240px, so the pinch is worst exactly where the split is narrowest and the grid is the half that needs width. Both floors were derived and neither is wrong; what was never decided is the *order* in which they give way, and `1fr` answers it by giving everything to the form first. The fix is to let the panel shrink towards its floor too — `minmax(0, 1fr) minmax(0, 620px)` and a `min-width` on the outline — which needs a look at these forms below 620px, the width they are only *known* to work at. **Waiting on:** the harness `&bounded=` row above, since judging a form at 560px means seeing where its fold is. |
| No boolean in the editor has ever been looked at | `src/test/obsidian-stub.ts` | The stub's `ToggleComponent` puts `.checkbox-container` on the `<input>` itself, where Obsidian nests the input inside a `<div class="checkbox-container">`. That one difference makes `editor.css`'s `.sheetsmith-component-form > .setting-item > .setting-item-control > input { width: 14em }` match the toggle — in the app it matches nothing, because there the container is the child and the input a grandchild, styled `opacity: 0`. So every boolean in the pane renders here as a 182px track with a native checkbox floating in the middle of it: visible in `editor-light`, `editor-dark`, `editor-stacked` and `editor-forced-colors`, and in the settings tab's own toggle before that. The instrument is harsher than the app rather than kinder, which is the safe direction and still means **`hideLabel`, `hideNote`, `signed`, `Show a total` and `Publish per row` have never actually been reviewed**. Not a plugin defect and not a stylesheet one: the fix is the stub emitting the app's own two-element markup and toggling `is-enabled` on the wrapper, which is the rule its own header already states — "the DOM shape matters and is not incidental". **Waiting on:** nothing but the work. |

| The layout editor pane has no narrow regime, and does not fit below about 470px | `styles/editor.css`, `editor/layout-editor.ts` | The pane's reflow rule stacks its two columns below 1176px and says nothing about anything narrower, so a pane in a sidebar or on a phone overflows horizontally — which in a leaf is a scrollbar dragging the tree and the panel sideways, not only the schematic. Seen in `editor-narrow`: the picker's delete icon is clipped, the schematic runs off the right, and the panel's `height` field is off-screen entirely. **Two independent contributors, each confirmed by hiding the other.** The `.setting-item` rows do not stack, and stacking them the way Obsidian stacks a settings row on mobile removes that half — verified with a throwaway override. The schematic's twelve-column grid overflows on its own with every row hidden. **Why it was never seen:** no editor shot went below 1190, and in a settings tab these same rows sat inside `.vertical-tab-content` where Obsidian's own narrow-settings CSS reaches them; a leaf never receives it, and `harness/calibrate.mjs` skips at-rules, so the harness carried none of it either and cannot say what the tab did. Not fixed here because only the first half is contained: the second is a design question the spec never asked — whether a twelve-column schematic at phone width shrinks to ~29px tracks, scrolls inside its own box, or is not drawn — and the stacking threshold has to be derived rather than borrowed, which is the failure §4 records. **Waiting on:** that decision, which is adjacent to the M4 canvas since it is about what a grid is for when it cannot be read. |

| The harness cannot show where the fold is, in any view | `harness/theme.css`, `harness/harness.ts` | The pane is deliberately allowed to grow — `.harness-editor .workspace-leaf-content` and `.view-content` are overridden to `height: auto` — so a shot captures the whole surface instead of one screen. That is the right default and the settings tab has always been shown the same way, but it means **no shot can show anything about scrolling, clipping, or what falls below the fold**. It has cost one real defect already: the pane shipped with a single scroller for both columns, so selecting a row near the bottom of a long tree scrolled the panel out of the leaf, and a 4600px full-page capture has no fold for that to be visible in. Two findings in one pass had to be checked with throwaway CSS overrides pasted into `index.html` instead. The fix is a `&bounded=` query that puts the leaf back to a real height for one shot, beside the existing `&open=` and `&layout=` — contained, and the only reason it is a row rather than a change is that it is instrument work and the pass that found it was about the pane. The reasoning also lives at the override in `theme.css`, which is where someone deleting it would look; this is where someone auditing the instrument would. **Waiting on:** nothing but the work. |
