...
- **A control names every party its context does not already announce**
  [judgement]. A move names where it goes, a remove names what it removes —
  unless the reader reached the control through something already named for
  it. The layout editor's tree is the first place this is written down: its row
  menu is opened by a button named `More options for "Weapons"`, so an item
...
- **Only a named level or an over-run value earns a `title`, and a tooltip
  repeating what is already legible is noise** fired at every pass [judgement].
  A two-state mark carries its state in `aria-pressed` and its name in
  `aria-label`, so a tooltip has nothing left to say, while a *named* level has a
  word the glyph cannot draw. Written down because the third component to draw a
  ring set `title` unconditionally.

  **The over-run half was added with
  `docs/features/track-stored-value-past-shortened-run.md`, and the premise
  above is why it fits rather than an exception to it.** A Track whose note holds
  more marks than its run draws the extra past the end, and a flag card holding a
  count above one reads as ticked; in both, how much the value holds is legible
  nowhere else — a slash to count on a run, nothing at all on a ring. The
  accessible description alone would leave a sighted reader unaware of it, and the
  ruling that a step down or an untick is an *informed* write depends on the reader
  having been told. So an over run titles its reading (`5 of 3, 2 over`), named or
  not, and a flag holding a count titles `Holds 3 marks from a longer run` through
  `bindRingControl`'s `note` option, since its paint owns the ring's `title`.
  ...
  **What "already legible" rests on is the control's name being on screen beside
  ...
  wrong, and it is not. **A record's name is on screen only in a headed list's
  wide regime** (`fieldHeadings`, and a list wide enough for the strip), and
  that is a width only the stylesheet sees, so a record's ring still answers
  `nameOnScreen: false` at every width: answering `true` would take the tooltip
  and the long press away in the narrow regime, where no name is on screen.
  **A ring inside the opened record answers `true`, and that is the same rule
  rather than an exception to it** (`docs/features/record-set-body-fields.md`):
  a body field draws its own name beside its control at every width and no strip
  or query touches it, so there the fact is knowable at draw time and it is true
  — Table's position. A body toggle has no tooltip; a body named level keeps its
  level's word.
...
| A control in the row position | `.sheetsmith-table-add`, `.sheetsmith-record-add` | Table's add row, Record set's add record — **one treatment**: a rule across the top, a centred label, and the hover surface every control on the sheet uses. **Not** a Track row set's own **Add** and **Remove**, `.sheetsmith-track-action-button`, which depart on purpose: one icon-only trigger per row set for each, Table's own delete glyph's circular shape, opening `ui/anchored-panel.ts`'s panel to name the specific row it offers rather than drawing a button per candidate or a bin icon per line — several *specific* things to add or remove, picked from a list rather than read off a row of controls (`docs/features/track-row-length.md`). **And the Add picker is a list *and a form*** where a Track lets the character add rows of their own: a **Name** field, a **Length** field and one **Add** button, in the panel's own `.sheetsmith-panel-field` clothes, appended under whatever declared rows are still on offer and separated from them by a rule rather than boxed — a form inside a panel is part of it, not a second surface floating in one. The name is typed *before* the entry exists, which is the whole reason it is a form rather than a press: the name is the fence key, so a default pressed twice would be a duplicate key, and that is a whole-section failure rather than a second row. Where nothing is left to offer the form is the whole panel, and the trigger is drawn whenever the toggle is on (`docs/features/character-added-track-rows.md`) |
...
| A list inside one row of another list | `editor/nested-list-field.ts`; `.sheetsmith-nested-list`, `-line`, `-controls`, `-controls-reserved` | a modifier definition's **Changes**. **The frame only**: the caller draws each line's own controls and names them, so the module knows that no modifier exists — `linked-text.ts`'s arrangement, in the editor. What it owns is the indent, the trailing control track, the remove, and the **Add** footer in `.sheetsmith-entry-footer`'s own clothes. **Indented by geometry rather than a background**, `--size-4-2`, so the nesting survives `forced-colors: active` where an author background is repainted one system colour — and little enough that a definition with one change is still the form that shipped. **A hairline between one line and the next, and spacing alone could not do it**: the forced break puts each line's six controls on two rows, measured 63px apart, against 56px between one line and the next — so the gap said the opposite of the grouping, and raising it far enough would make a two-line entry twice the object a one-line one is. A rule is a difference in kind, it survives forced colors as a border, and it costs a pixel. **The remove is reserved and hidden on the only line there is**, `.sheetsmith-detail-field-reserved`'s argument one element over: a control that is not created gives its width back to the line's grow, so a list holding one-line and two-line entries would read as two forms down the pane. **No reorder and no drag**, which is a refusal: order inside this list is not observable, since a breakdown lists contributors in *definition* order and two changes at one target contest by size (`docs/features/multi-change-definitions.md`) |
...
| A value read against its ceiling | `.sheetsmith-pool-ceiling`, `-separator`, `-max` | Pool's max, a Record set `number` field whose ceiling the layout declares or the record holds — **one vocabulary**: the value at full contrast, a `/`, and the ceiling muted with tabular figures. A record overrides the size only, since the pool's ceiling qualifies a headline number and a record's a 13px one; where the ceiling is the record's it is a *field* rather than a span, wearing the record's own chrome and this reading. **A Track row whose length the character types takes only the ceiling half** (`.sheetsmith-pool-max`, `.sheetsmith-pool-max-input`), narrowed again to its own row-name scale (`.sheetsmith-track-row-length-input`) — there is no value beside it to separate from, since a run's segments are already the reading of the value the way Pool's own numeral is the reading of its own (`docs/features/track-row-length.md`) |
| A number something has been pushed at | `.sheetsmith-modified`; `components/modifier-breakdown.ts` | Card's and Card set's `derived`, Table's computed cell, **and a Track's run** — which takes the *builder* and deliberately not the underline: `text-decoration` draws nothing on a flex row of spans and would fight the slider's own cursor, so a run's mark is the granted segments themselves and the builder's text is what explains them. The two carriers are the card's spelling rather than the cell's — a run is one focusable control, so a `title` and a `.sheetsmith-sr-only` twin it points at with `aria-describedby` hold one string, composed once (`docs/features/modifier-granted-track-segments.md`) |
| A door onto a breakdown, where the number's own press is taken | `.sheetsmith-track-modifier-button`; `ui/popover.ts`'s `showPopover` | Track. **The third door onto one builder and the first that is not the number itself**: a Card opens its breakdown from the `derived` and a computed cell from the cell, and a Track cannot, because a press on the run sets the value. So the affordance is a glyph-only `<button>` beside the card's name — beside the *label* and not in the run's row, which is the constraint rather than a preference, since everything on that row is a target or something a drag passes over. `<button>` is also what makes Enter and Space arrive as a click, so there is one route in and no second code path. It takes the treatment of the row set's own pickers by selector list rather than by a copy, so three glyph buttons on one card cannot measure differently under one finger, and it takes `--text-muted` on the modifier cell's own measured argument — a faint bolt at this size reads as a smudge. **A fourth figure on a component that already draws three**, and a glyph rather than a fourth box treatment is why it does not collide: a plain square, a granted ring and a blocked slash are three ways of drawing one box |
| A segment a modifier granted | `.sheetsmith-track-segment-granted` and its `::after` | Track's run, at the far end of it. **Two geometric channels and no colour**, which is §1 and the forced-colors rule rather than a preference: a dash and a wider gap at the join both survive `forced-colors: active`, where that mode strips author border and background *colours* and forces `box-shadow` to none. The convergent prior art separates a raised ceiling by fill colour alone over one flat outline, which is WCAG 1.4.1 failure technique F13. **The dash is an overlay and not a border, and the box model is why**: a segment's fill is an absolutely positioned child occupying the padding box, so a dashed *border*'s gaps show the card rather than the fill and a filled segment drew with nibbles bitten out of its edges — 2.12:1 against the card and 1.49:1 against the fill, measured at 15px. A positioned `::after` paints after its element's other positioned children, so the ring lands on the fill and its gaps show accent. An `outline` outside the box is the other obvious answer and overlaps between two consecutive granted segments. **The segment's border width becomes padding**, which is the whole of how the ring paints at the base segment's own footprint: a positioned child is laid out against the *padding* box and `overflow: hidden` clips to it, so moving 1.5px from border to padding grows that box to the whole square without moving the square. Holding the border `transparent` instead was the first answer and drew 13px inside a 15px box — layout identical, picture not, with the join reading 9 against 8. It also leaves nothing for forced colors to repaint into a second ring, so the overlay is the mark in every mode. `--text-muted` because a ring on a different element inherits no base rule and a border that alone marks a state clears 3:1. **Filled and empty are two different-looking marks**, not one seen twice: the ring measures 1.95:1 over the accent, so a filled granted segment reads as ringed and an empty one as dashed |
| A slot a modifier took away | `.sheetsmith-track-segment-blocked`'s `::after` | Track's run, at the far end, drawn where it has always been. **Present and unusable, which is not a ghost**: a ghost faintly shows what is *not* there and puts a second ceiling on screen, where this is a first-class state a reader can count — the convergent prior art asks for it outright (foundryvtt-lancer 548 wants "a slash through the icon" on a spent pip). **A third state on one row, so the figure changes rather than the line style**: solid against dashed against dotted at 15px is one mark a reader can name and two they cannot, and a slash reads against both a base square and a granted ring. A rotated `border-top`, because forced colors strips a background and recolours a border; the bar is longer than the box and the segment's own `overflow: hidden` clips it to a clean corner-to-corner diagonal. **It is not a target and nothing refuses a press**: the hit test is handed the live segments' rectangles only, so an empty blocked slot is not a position the control has. A blocked slot that still holds a mark is the row below |
| A segment past the live run that holds marks | `.sheetsmith-track-segment-over`, sharing `.sheetsmith-track-segment-blocked::after` by selector list; `.sheetsmith-track-over-count` past `MAX_SEGMENTS` | Track's run, after the live segments — a count lowered, a formula resolving lower, a grant removed, a penalty over held marks, a character's row shortened, all one state. **The blocked slash on the existing fill, and not a fourth figure**: a slot past what the run can take is one figure whatever put it there, and the fill is the one difference, as it is between a lit and an unlit base segment — the owner's condition for drawing it at all. It is in the hit test, so a reader steps down through it and nothing steps up past the stored value; `aria-valuemax` is the stored value and `aria-valuetext` says the rest, `5 of 3, 2 over`. Past `MAX_SEGMENTS` it is one box holding its count in marks, `+147`, with no slash across the digits (`docs/features/track-stored-value-past-shortened-run.md`) |
...
| Which glyph a modifier surface takes | `zap` / `zap-off` against `info` | **`zap` is the modifier-*authoring* glyph and `info` is the read-only account of one.** Every bolt on a sheet opens something a reader can change — a modifier cell's picker, the form behind it — and a Track's breakdown button opens something they can only read, so it takes `info`. Written down because the choice was made twice without being: the bolt is the better *association*, since a reader learns "what is affecting this" from the modifier column three cards up the same screen, and it is the worse *affordance*, since it teaches a press that edits. A third consumer picks a side from this row rather than from whichever surface it happened to read first |
| A disclosure that opens a block in place | `.sheetsmith-record-disclosure`, `.sheetsmith-record-body` | Record set's records, **and the layout editor's tree** (`.sheetsmith-tree-disclosure`): the same `chevron-down` / `chevron-right` pair, `aria-expanded` and `aria-controls` (left off an open empty container, which draws no region to name), a label that changes with the state (**Collapse "Weapons"** / **Expand "Weapons"**) and `--text-muted` for the same measured reason. What differs is the body: a shut container's children are not rendered at all rather than `hidden="until-found"`, since the tree is not find-in-page content and the tree opens the ancestors of whatever is selected (`docs/features/layout-editor-tree.md` §3) |
| A tree's nesting | `editor/tree.ts`; `.sheetsmith-tree-children`, `.sheetsmith-tree-slot` | the layout editor's tree. **The card steps in, not its text**: a container's rows render into a `role="group"` wrapper named `Inside <container>`, indented as a whole, and the wrapper's own `border-inline-start` is the guide, so it runs the full height of what the container holds, gaps included, and two containers deep draws two. Placed under the centre of the container's chevron. A border rather than a background, so forced colors repaints it. The slot before every component row's name holds the chevron or an empty spacer of its width, so names at one depth share a column (`docs/features/layout-editor-tree.md` §2) |
| A record's summary line | `.sheetsmith-record`, `-summary`, `-name`, `-fields`, `-field` | Record set. **On a headed list wide enough for the strip the fields sit on column tracks shared across every record** — the list is one grid and each record, summary and fields wrapper a subgrid of it, so a heading centres over its field whatever the records hold — and below the threshold the line is the one it always was (`docs/features/record-set-heading-strip.md`). **A field placed in the body is not on this line at all**: it draws in `.sheetsmith-record-body-fields`, a wrapping flex block that is the opened body's first child, with no track and no heading, and the body gains `-has-fields` and a second grid row only when it holds one (`docs/features/record-set-body-fields.md`) |
...
| An inline disclosure with a pinned action bar | `editor/component-picker.ts`; `.sheetsmith-picker`, `-search`, `-list`, `-group`, `-option`, `-entry`, `-name`, `-description`, `-preview`, `-example`, `-empty`, `-bar`, `-controls`, `-status` | the layout editor's component picker. **A block in the pane's own flow, not a floating surface**: clothed like the list fields (`.sheetsmith-entry-list`'s border, radius and secondary surface), directly under the row that opens it, so it is not a fifth kind of panel and uses none of `ui/anchored-panel.ts`. What is new is the bar: `position: sticky; bottom: 0` against whichever box scrolls the pane, so **Add** stays in reach while the list runs past the viewport and sits at the picker's foot once its end is on screen. The active line draws one inert preview of itself through the canvas's own render path, and an **Example** tag in the flair's quiet type where the drawing is not what **Add** writes (`docs/features/component-picker.md`) |
...
**That row is history now**: the component picker replaced its dropdown
(`docs/features/component-picker.md`), and describes every line in its own list,
so it has no row to move a description under. The class keeps its name and its
two later consumers, below.
