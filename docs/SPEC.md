...
- **How a Table publishes its *declared* rows to the rest of the sheet.** `10 + skills.perception` is the obvious way to write a passive perception, and a Table publishes its column totals and nothing else, so it cannot be written that way yet. The question is now bounded to declared rows: a row the character added publishes nothing at all, because a name a formula can write has to be stable and knowable when the formula is written (§4.2), and what an open list publishes instead is a column total. The obstacle is mechanical rather than a matter of taste. A row's published value is a computed column evaluated in a scope that itself contains formulas (the row's `values`), and `scopeValues` (§4.1) hands the sheet plain values, having no resolver of its own at the time it is called. Either the scope entry grows a way to name an expression to evaluate, or a component gains a say in how its resolver is built. Until then a per-row value is readable on the sheet and not from a formula. A **column** total is not blocked by any of this and is built: a sum over a stored column is a number derived from data alone, so `ScopeEntry.value` carries it with no change to the contract, and that boundary is also why a `total` on a computed column is refused. A Track's rows are the second instance, and they add a second shape of the same problem: a row publishes `slots.L1`, its ceiling would be `slots.L1.count`, one level deeper than `<id>.<name>` reaches, and the total across the rows is not a name at all, so "how many slots are left" cannot be written even though it is the first formula a slot card invites.
