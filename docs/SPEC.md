...
**Stat group**. An ordered set of named attributes rendered as a strip of stat cards. Covers the six D&D abilities, Call of Cthulhu characteristics; a single-attribute group is a lone stat card. A currency block is this component with its denominations as attributes, offered as a palette entry rather than a type of its own (§13).
...
- **Grid canvas**: a responsive grid. Drag components in from a palette, move them, resize by grid units. The palette offers entries rather than types (§2): one type appears in it several times with configuration prefilled, as Table does under Inventory and Features and Stat group under Currency, and dropping one in writes an ordinary component the author then edits (§13). Grid rather than free positioning, so sheets reflow to a single column on a phone instead of breaking.
...
What it does not settle: the depth question above, which is still what "how many slots are left" waits on. And it does **not** deliver item modifiers, despite the board saying that card waits on this one. The prior art is a counter-example worth acting on — both Foundry builders answer item modifiers by *push*, a row declaring a modifier against a named target, and one of them is deprecating its own pull-side system in favour of that — because a pull aggregate requires the target to enumerate every source that could modify it, which is the thing push exists to avoid. So that card's dependency deserves re-examining rather than acting on.

Resolved: **the five blocks still on the list are three palette entries and two layout patterns, and the catalog does not grow.** A currency block is a Stat group with its denominations as attributes; an inventory and a features list are Tables with `openRows` on and their columns prefilled; a spellbook and a stat with its skills are several components placed together, which is an example layout rather than anything the editor offers as one item. §4.2 gains no entry, and none of the five wanted a capability the catalog lacks.
