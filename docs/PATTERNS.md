...
| The create-element-with-class primitive is spelled three times, and the three disagree on mechanism | `src/ui/element.ts` (owed), `components/modifier-form.ts`, `components/table.ts`, `components/record-set.ts` | Three copies of "make an element, give it a class, put text in it, append it": `modifier-form.ts` assigns `el.className = cls`, while `table.ts` and `record-set.ts` split on a space and call `classList.add` per class. §1 extracts at three, and the *disagreement* is what makes this more than tidiness — **`classList.add` throws `InvalidCharacterError` on a string with a space in it, `src/test/obsidian-stub.ts` accepts it, and that exact pair once aborted a render mid-loop so the sheet lost every row below the first while 2139 tests stayed green** (`docs/UI.md` §12 holds the general form of that trap). The `className =` copy is immune by accident rather than by decision, which is the worst of the three states to be in: a call site moved between the two spellings changes behaviour silently. **The fix is `src/ui/element.ts`** — `ui/` rather than `components/`, because nothing about it knows what a component is, and that keeps it off the sibling allowlist — **plus the guard §10 asks for: a scan for a bare `classList.add` whose argument literal holds a space**, which is narrow enough to only ever report what it can prove. **Not taken in the pass that found it**, on `docs/UI.md` §12's `.dropdown` precedent: the fix edits two components this feature otherwise does not open, in a diff already past +1700, and an unrequested rewrite of how two existing components build DOM is exactly the change an owner should see on its own. **Waiting on:** a pass that does only this, or a fourth copy — a fourth is the same argument at a worse count. |
| A single-line field's refusal is hand-rolled, where the multi-line one has a policy | `components/record-set.ts`, `interaction/editable.ts` | `bindMultiline` takes a `refuse`/`onRefusal` pair whose comment states the policy outright — "**the draft is kept, not discarded**… a refusal that hid what the reader typed would be the silent loss this exists to stop, wearing a message" — and a record's *body* uses it. A record's *name* refuses a blank commit and does the opposite: it calls `handle.sync()` to put the stored name back, discarding what was typed, and announces what was kept. **The behaviour is right and is not what this row asks to change**: the only refusal a single-line field has here is a blank name, so "keeping the draft" means leaving an empty field on screen with a message saying it was not saved, which is worse than showing the name that is actually stored. What is missing is that `editable.ts` states one policy for a refused draft and the repository holds two, with nothing saying the single-line case is deliberately the other one. **The fix, when it is owed:** `EditableOptions` gains `refuse` and `onRefusal` alongside `bindMultiline`'s, with the draft-keeping made a caller's choice rather than the binding's, and this call site opts out with its reason. **Waiting on:** a second single-line refusal anywhere — which is where §1 extracts, and where the question of which way the policy points stops being answerable by one caller's convenience. |
