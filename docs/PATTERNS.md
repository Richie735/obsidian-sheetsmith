...
| Gap | Where | Fix |
| --- | --- | --- |
| A gesture engine and the layout editor pane in one file, and the gesture half is untested | `layout-editor.ts` | Three seams, and all three are now taken. **The pane took the tree from the form**: the tree of what the layout holds and the panel configuring the selected one are two regions, drawn into two elements, which is what closed `docs/UI.md` §12's open-container row — a form drawn under its own row cannot get out from between a container and its children. **The two slices after it took the field code**: `renderEntriesEditor` went to `list-fields.ts`, which was already lending it the chrome it drew with — the move itself was argued and taken in that pass rather than carried over from an earlier one. The reset binding became `reset-field.ts` beside `trigger-list-field.ts`, which is the other half of SPEC §6. `onCommit` went to `field-commit.ts` on the way, because the reset field was its second caller and a commit policy spelled twice is the drift nothing reports, and `showFieldError` went to `field-error.ts` behind it, because the reset field made it three modules deep and a file whose job is list fields cannot also be the home of the editor's error vocabulary — `copyableName` went with it to `copyable-name.ts` on the atomicity half of that argument alone, not the ladder. **The field halves came out uncovered, and no longer are.** Promoting a private method to a named module is the moment §10 attaches to it, and neither half of §6 had a test file: `reset-field.ts` arrived without one in this pass — 276 lines whose three refusals, disabled add and remove control nothing drove — and the 158 of `trigger-list-field.ts` had been uncovered since `a95c590` gave the editor its folder, its `commitTriggerList` gating every write the editor makes — answer true for an unchanged list and a pointerdown on the schematic rewrites the file. The pane could only ever assert that each field was *on screen*, which is what its cases do. Both have their own test file now, mutation-checked rather than merely green. **What is left is the schematic's gesture layer**, and it was always the half carrying the real debt: around 350 lines of drag, resize and nudge arithmetic — `beginDrag`, `nudge`, `previewMetrics`, `cellAt`, `markOverlaps` — with no behavioural coverage at all, only the pure arithmetic under it in `preview-grid.ts`. Two problems, not one, and the order matters: it is a second responsibility in this file *and* it is untested, and moving untested gesture code to `interaction/` would only relocate the risk. `pool.ts` is the worked example in §1 and it is the precedent for both halves — the engine moved, and the class names stayed with the caller. **Waiting on:** the coverage first, then the move. A test that presses a drag on a schematic block and asserts where it lands is what makes the extraction safe, and it is the piece the pane slice deliberately did not add, with the cost stated in `docs/features/layout-editor-pane.md`. |
