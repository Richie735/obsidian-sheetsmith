...
| Gap | Where | Fix |
| --- | --- | --- |
| Three responsibilities in one file, two of them still joined | `layout-editor.ts` | Three seams, and the workspace pane took the one it could not avoid. **Taken:** the tree from the form. The tree of what the layout holds and the panel configuring the selected one are two regions now, drawn into two elements, which is what closed `docs/UI.md` §12's open-container row — a form drawn under its own row cannot get out from between a container and its children. **Not taken, and each is a slice of its own:** `renderEntriesEditor` moving to `list-fields.ts`, where that module's own header already says it belongs; and the reset binding becoming its own module. The schematic's gesture layer goes with them, and it carries the real debt — around 350 lines of drag, resize and nudge arithmetic with no behavioural coverage at all, only the pure arithmetic under it in `preview-grid.ts`. The pane slice moved that code into a new host without adding a test to it, deliberately, with the cost stated in `docs/features/layout-editor-pane.md`. **Waiting on:** nothing but the work. What changed is that it is no longer waiting on a design decision. |
