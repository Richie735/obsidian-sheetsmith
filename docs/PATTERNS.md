...
| §10's editor-module exception reads as settled practice, and the practice is split | `docs/PATTERNS.md` §10, `editor/function-library-field.ts` | §10's second exception enumerates the small modules driven through `layout-editor.test.ts` and rests the case on "each says so in its own header". The roster is not what that sentence implies: `trigger-list-field.ts`, `reset-field.ts` and `list-fields.ts` have their own test files, while `function-library-field.ts` — the same shape, the same entry point, extracted in the same pass as the trigger list — does not, and **nothing in either file or in §10 distinguishes them.** So an author adding a fourth field module has no rule to follow and has to guess; the item-modifiers pass had to, guessed "own test file" from the trigger list, and was right by luck rather than by reading anything. **Found from outside, in a review of that pass, and recorded rather than fixed because the fix is a decision about the exception's boundary and not about a missing file.** The candidate rule is that a module with its own entry point and its own reportable output earns a file, and the five true fixtures — `accepts-children.ts`, `component-name.ts`, `copyable-name.ts`, `field-commit.ts`, `form-group.ts` — have neither, being reached only by pressing something the editor drew. That rule is checkable and would oblige one new test file (`function-library-field.test.ts`), which is work this row is not doing; it would also want §10's clause rewritten, since "each says so in its own header" is the evidence that turned out not to hold. **What is not waiting on that decision, and is done:** `editor/field-lines.ts` carries its own file, because half of what it owns — the trimEnd rule the function library alone speaks — had no driver anywhere and §10's own condition on every exception is that what a module owns has to actually be driven somewhere. **Waiting on:** a decision on where the exception's line falls, taken as its own diff with the test file it obliges. |
