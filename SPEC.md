...
- *Config:* `label`, `count`, `marks`, `levels[]`, `sense`, `hideLabel`, `reset`
- *Data:* `fenced`, a single value under `value`, counted in marks
- *Sheet view:* the run is one control. Press a segment to set the track there, press the highest filled segment to clear it, drag across the run to set it in one gesture, arrows to step. The segments are the reading, so there are no numerals beside them
...
- **Whether two opposed runs justify a component.** Death saves are two Tracks in a Group by §4.2's argument. If systems keep producing runs that resolve against each other — and tug-of-war clocks are a second example — the answer may be a config field rather than a placement.
