...
- *Sheet view:* increment, decrement, direct entry, and a drag on the number itself, so damage and healing are one action. The step buttons sit either side of the number and move it by one, or by ten with shift, and repeat while held. A press steps what is on screen rather than what was last saved, so a typed draft is what moves
