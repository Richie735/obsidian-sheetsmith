---
name: design-review
description: "Use when reviewing how a feature looks and feels, after it works. Reviews the rendered harness against docs/UI.md and the feature's spec. Requires looking at the UI, not reading CSS."
...
both surfaces, the narrow reflow, and the empty and error states.
...
- `harness:calibrate` extracts the real theme palette and settings chrome from
  ...
  hand-written fallback in `harness/theme.css` — say so in your report if you
  had to.
...
For a question the harness cannot answer — how a community theme treats this, or
whether the real app agrees — `harness/inspect.mjs` drives the running Obsidian
